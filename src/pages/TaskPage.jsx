...
export default function TaskPage() {
  ...
  const tabClass = (tab) =>
    activeTab === tab ? "bg-[#06b6d4] text-white" : "bg-white/4 text-[#9ca3af]";
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center justify-center w-5 h-5 rounded-full flex-shrink-0 bg-[rgba(6,182,212,0.25)]">
                        {activeTab === "ads" && <DollarSign size={12} strokeWidth={2} color="#ffffff" />}
                        {activeTab === "survey" && <BarChart3 size={12} strokeWidth={2} color="#ffffff" />}
                        {activeTab === "game" && <Gamepad2 size={12} strokeWidth={2} color="#ffffff" />}
                      </span>
                  ...
                  <div className="text-right text-sm">
                    ...
                    <div className="text-xs text-[#9ca3af] mt-0.5">{task.duration}</div>
                  </div>
                </div>
                ...
                <div className="mt-3 flex items-center gap-3">
                  {task.status === "available" && (
                    <button
                      onClick={() => alert(`Task opening for: ${task.title}`)}
                  ...
                  {task.status === "pending" && (
                    <span className="text-xs text-amber-500 flex-1">
                      <Sparkles size={14} strokeWidth={2.2} className="inline mr-1" />
